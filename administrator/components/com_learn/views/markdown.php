...
abstract class ComLearnViewMarkdown extends KViewAbstract
{ 
    /**
     * Document identifier (markdown://APP/directory.COMPONENT.template.NAME)
     *
     * @var string|object
     */
    protected $_document;
    ...
    public function __construct(KConfig $config)
    {
        ...
        if(!empty($config->document_filters)) {
            $this->getDocument()->addFilter($config->document_filters);
        }
        ...
        $this->getDocument()->getFilter('alias')->append(
    ...
    */
    ...
    public function display()
    {
        if(empty($this->output))
		{
		    ...
		    $this->output = '<div>'.Markdown($this->getDocument()
    ...
    {
        if(!$this->_document instanceof KTemplateAbstract)
        { 
            ...
            $this->_document = KFactory::get($this->_document, $options);
        }
    ...
    {
        if(!($document instanceof KTemplateAbstract))
        {
			...
			else $identifier = KFactory::identify($document);
